...
- When `$ARGUMENTS` is not empty, use it as context to push commit messages, but do not force that text if it does not accurately describe the changes.
- Use clear, semantic, concise commit messages that follow the repo's recent style.
- Before commiting, check for sensitive of suspicious files (`.env`, tokens, credentials, keys, secrets). If any appear, stop and ask.
...
3. For each group:
    - Add only the files for tat group with `git add <files...>`.
    - Create the commit with semantic message.
4. Once all commits have been created, run: `git push`
5. When finished, summarize the commits created and the files were pushed.
